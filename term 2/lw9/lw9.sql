...
begin
    update auditorium
    set auditorium_capacity = 'qwerty' where auditorium = '206-1';
exception
...
--TASK 15 как параметр в функцию
declare
...
declare
    cursor curs_aut is
        select auditorium_type, cursor
            (select auditorium from auditorium aum where aut.auditorium_type = aum.auditorium_type)
...
select * from auditorium order by auditorium;
 --в PL/SQL используется для обновления или удаления записи, которая была последней выбрана курсором1. Это позволяет
 -- обновлять или удалять строки непосредственно из результирующего набора курсора1.
 ...
 declare
    cursor curs_auditorium(capacity auditorium.auditorium%type, capac auditorium.auditorium%type)
        ...
        where auditorium_capacity >= capacity and AUDITORIUM_CAPACITY <= capacity
            for update;
    aum auditorium.auditorium%type;
...
select * from auditorium order by auditorium;
-- Конструкция FOR UPDATE в операторе SELECT используется в PL/SQL для блокировки выбранных строк с целью последующего обновления
--TASK 18
select * from auditorium order by auditorium;

--ROWID в Oracle представляет собой псевдостолбец, который возвращает адрес каждой строки в базе данных1.
-- Значения ROWID содержат информацию, необходимую для определения местоположения строки: номер объекта
-- данных объекта, блок данных в файле данных, в котором находится строка, позицию строки в блоке данных
-- (первая строка - 0), файл данных, в котором находится строка (первый файл - 1)1.
